import 'package:ipray/models/users_models.dart';

class VariablesPhrases {
  List<BibleVerse> bibleVerses = [
    BibleVerse(
      livro: "Salmos",
      frase: "O Senhor é o meu pastor, nada me faltará.",
      capitulo: 23,
      versiculo: 1,
    ),
    BibleVerse(
      livro: "Isaías",
      frase:
          "Não temas, porque eu sou contigo; não te assombres, porque eu sou o teu Deus; eu te fortaleço, e te ajudo, e te sustento com a destra da minha justiça.",
      capitulo: 41,
      versiculo: 10,
    ),
    BibleVerse(
      livro: "Salmos",
      frase:
          "Deem graças ao Senhor, porque ele é bom; o seu amor dura para sempre.",
      capitulo: 107,
      versiculo: 1,
    ),
    BibleVerse(
      livro: "Lucas",
      frase: "Porque para Deus nada é impossível.",
      capitulo: 1,
      versiculo: 37,
    ),
    BibleVerse(
      livro: "Filipenses",
      frase: "Tudo posso naquele que me fortalece.",
      capitulo: 4,
      versiculo: 13,
    ),
    BibleVerse(
      livro: "1 João",
      frase: "Deus é amor.",
      capitulo: 4,
      versiculo: 8,
    ),
    BibleVerse(
      livro: "Provérbios",
      frase:
          "Confia no Senhor de todo o teu coração, e não te estribes no teu próprio entendimento.",
      capitulo: 3,
      versiculo: 5,
    ),
    BibleVerse(
      livro: "1 Coríntios",
      frase:
          "O amor é paciente, o amor é bondoso. Não inveja, não se vangloria, não se orgulha.",
      capitulo: 13,
      versiculo: 4,
    ),
    BibleVerse(
      livro: "Mateus",
      frase:
          "Bem-aventurados os humildes de espírito, porque deles é o Reino dos céus.",
      capitulo: 5,
      versiculo: 3,
    ),
    BibleVerse(
      livro: "Filipenses",
      frase:
          "Não andem ansiosos por coisa alguma, mas em tudo, pela oração e súplicas, e com ação de graças, apresentem seus pedidos a Deus.",
      capitulo: 4,
      versiculo: 6,
    ),
    BibleVerse(
      livro: "Jeremias",
      frase:
          "Porque sou eu que conheço os planos que tenho para vocês', diz o Senhor, 'planos de fazê-los prosperar e não de lhes causar dano, planos de dar-lhes esperança e um futuro.",
      capitulo: 29,
      versiculo: 11,
    ),
    BibleVerse(
      livro: "João",
      frase:
          "Se vocês permanecerem em mim, e as minhas palavras permanecerem em vocês, pedirão o que quiserem, e lhes será concedido.",
      capitulo: 15,
      versiculo: 7,
    ),
    BibleVerse(
      livro: "Mateus",
      frase: "Vocês são a luz do mundo.",
      capitulo: 5,
      versiculo: 14,
    ),
    BibleVerse(
      livro: "Mateus",
      frase:
          "O amor é paciente, o amor é bondoso. Não inveja, não se vangloria, não se orgulha.",
      capitulo: 13,
      versiculo: 4,
    ),
    BibleVerse(
      livro: "Salmos",
      frase:
          "O Senhor é bom, um refúgio em tempos de angústia. Ele protege os que nele confiam.",
      capitulo: 1,
      versiculo: 7,
    ),
    BibleVerse(
      livro: "Mateus",
      frase:
          "Portanto, não se preocupem com o amanhã, pois o amanhã trará suas próprias preocupações. Basta a cada dia o seu próprio mal.",
      capitulo: 6,
      versiculo: 34,
    ),
    BibleVerse(
      livro: "1 Pedro",
      frase:
          "Humilhem-se, portanto, sob a poderosa mão de Deus, para que ele os exalte no tempo devido.",
      capitulo: 5,
      versiculo: 6,
    ),
    BibleVerse(
      livro: "Romanos",
      frase:
          "Antes, em tudo isto, somos mais do que vencedores, por meio daquele que nos amou.",
      capitulo: 8,
      versiculo: 37,
    ),
    BibleVerse(
      livro: "João",
      frase:
          "O meu mandamento é este: amem-se uns aos outros como eu os amei. Ninguém tem maior amor do que aquele que dá a sua vida pelos seus amigos.",
      capitulo: 15,
      versiculo: 12,
    ),
    BibleVerse(
      livro: "Romanos",
      frase:
          "Pois todos pecaram e carecem da glória de Deus, sendo justificados gratuitamente por sua graça, por meio da redenção que há em Cristo Jesus.",
      capitulo: 3,
      versiculo: 23,
    ),
    BibleVerse(
      livro: "Gálatas",
      frase:
          "Já estou crucificado com Cristo; e vivo não mais eu, mas Cristo vive em mim; e a vida que agora vivo na carne vivo-a pela fé do Filho de Deus, que me amou, e se entregou a si mesmo por mim.",
      capitulo: 2,
      versiculo: 20,
    ),
    BibleVerse(
      livro: "Mateus",
      frase:
          "Venham a mim, todos os que estão cansados e sobrecarregados, e eu lhes darei descanso.",
      capitulo: 11,
      versiculo: 28,
    ),
    BibleVerse(
      livro: "2 Coríntios",
      frase:
          "Porque as armas da nossa milícia não são carnais, mas sim poderosas em Deus para destruição das fortalezas.",
      capitulo: 10,
      versiculo: 4,
    ),
    BibleVerse(
      livro: "Hebreus",
      frase:
          "Ora, a fé é o firme fundamento das coisas que se esperam, e a prova das coisas que não se veem.",
      capitulo: 11,
      versiculo: 1,
    ),
    BibleVerse(
      livro: "Efésios",
      frase:
          "Porque somos feitura sua, criados em Cristo Jesus para boas obras, as quais Deus antes preparou para que andássemos nelas.",
      capitulo: 2,
      versiculo: 10,
    ),
    BibleVerse(
      livro: "Tiago",
      frase:
          "Se algum de vós tem falta de sabedoria, peça-a a Deus, que a todos dá liberalmente, e não o lança em rosto, e ser-lhe-á dada.",
      capitulo: 1,
      versiculo: 5,
    ),
    BibleVerse(
      livro: "1 João",
      frase:
          "Se dissermos que não temos pecado, enganamo-nos a nós mesmos, e não há verdade em nós.",
      capitulo: 1,
      versiculo: 8,
    ),
    BibleVerse(
      livro: "Provérbios",
      frase:
          "O temor do Senhor é o princípio da sabedoria, e o conhecimento do Santo a prudência.",
      capitulo: 9,
      versiculo: 10,
    ),
    BibleVerse(
      livro: "1 Pedro",
      frase:
          "E, chegando-vos para ele, pedra viva, rejeitada, na verdade, pelos homens, mas para com Deus eleita e preciosa.",
      capitulo: 2,
      versiculo: 4,
    ),
    BibleVerse(
      livro: "Mateus",
      frase: "Assim também a fé, se não tiver obras, é morta em si mesma.",
      capitulo: 2,
      versiculo: 17,
    ),
    BibleVerse(
      livro: "Salmos",
      frase: "O Senhor é o meu pastor, nada me faltará.",
      capitulo: 23,
      versiculo: 1,
    ),
    BibleVerse(
      livro: "Salmos",
      frase: "O Senhor é o meu pastor, nada me faltará.",
      capitulo: 23,
      versiculo: 1,
    )
  ];
}
